...
const personalInfo = {
      ...
      <div class="project-card">
        ...
        <div class="project-content">
          ...
          <a href="#" class="project-link">View Details
            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-left:0.25rem;"><path d="M15 2h5v5"/><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"/><path d="M15 2 8 9"/></svg>
          </a>
        </div>
      </div>
    `).join('');
  ...
  function renderAchievements(){
    ...
    container.innerHTML = achievements.map((item, index) => {
      ...
      if(isDesktop){
        contentClass += (index % 2 === 0) ? ' float-right' : ' float-left';
      }
      return `
        <div class="timeline-item">
          <div class="timeline-dot" style="border-color: var(--color-bg);">${item.icon}</div>
          <div class="timeline-line"></div>
          <div class="achievement-card ${contentClass}">
            <span>${item.date}</span>
            <h3>${item.title}</h3>
            <p>${item.detail}</p>
            <p>${item.conf}</p>
